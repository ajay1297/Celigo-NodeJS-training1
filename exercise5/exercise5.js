...
const csvWriter = createCsvWriter({
    ...
    header: [
        {id: 'url', title: 'URL'},
        {id: 'shortendUrl', title: 'SHORTEND URL'}
    ]
});
...
let shortendUrl = function(url, callback) {
    shortUrl.short(url, (error, shortendUrl) => {
        if(error) {
            callback(error);
            return ;
        }
        shortendUrls.push({url, shortendUrl});
        callback(null, {url, shortendUrl});
    });
}
async function process(urls) {
    try {
        let data = await async.each(urls, shortendUrl);
        console.log(shortendUrls);
        await csvWriter.writeRecords(shortendUrls)
    }
    catch(error) {
